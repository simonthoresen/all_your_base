...
public class TileMap {
    ...
    public void paint(Graphics2D g, Rectangle viewport, Rectangle2D mapRegion) {
        ...
        Point cursor = new Point((int)(((int)mapX - mapX) * tile.width), (int)(((int)mapY - mapY) * tile.height));
        Rectangle region = new Rectangle(
                (int)mapX, (int)mapY,
                Math.min((int)Math.ceil((viewport.width - cursor.x) / (double)tile.width),
                         bounds.width - (int)mapX),
                Math.min((int)Math.ceil((viewport.height - cursor.y) / (double)tile.height),
                         bounds.height - (int)mapY));
